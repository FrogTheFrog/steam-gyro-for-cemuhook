...
import { ClientTimeoutLimit, MaxProtocolVer } from "../models/const/udp-server.const";
import { UdpServerMessage } from "../models/enum/udp-server-message.enum";
...
import { UdpServerEvents } from "../models/interface/udp-server-events.interface";
...
export class UdpServer {
    ...
    private controllers: Array<{
        device: DualshockGenericController,
        subscription: Subscription,
    } | null> = new Array(4).fill(null);
    private clients = new Map<AddressInfo, ClientRequestTimes>();
    private eventSubject = new Subject<GenericEvent<UdpServerEvents>>();

    public start(port?: number, address?: string, callback?: () => void) {
        this.stop();
    ...
    public addController(controller: DualshockGenericController) {
        ...
        for (let i = 0; i < this.controllers.length; i++) {
            if (this.controllers[i] === null) {
                ...
                this.controllers[i]!.subscription.add(controller.events.subscribe((data) => {
                    switch (data.event) {
                        case "error":
                            this.emitError(data.value);
                            break;
                        case "dualshockData":
                            this.handleReport(data.value);
                            break;
                        default:
                            break;
                    }
    ...
    private sendPacket(
        ...
        data: Buffer,
        protocolVer: number = MaxProtocolVer,
        callback?: () => void,
    ) {
        ...
        this.socket!.send(buffer, clientEndpoint.port, clientEndpoint.address, (error, bytes) => {
            if (error) {
                ...
                this.emitError(new Error(`failed to completely send all of buffer. Sent: ${bytes}. Buffer length: ${buffer.length}`));
            }
    ...
    private onMessage(data: Buffer, clientEndpoint: AddressInfo) {
        try {
            if (data[0] === char("D") && data[1] === char("S") && data[2] === char("U") && data[3] === char("C")) {
                ...
                const computedCrc = crc.crc32(data);
                ...
                if (msgType === UdpServerMessage.DSUC_VersionReq) {
                    ...
                    for (let i = 0; i < numOfPadRequests; i++) {
                        ...
                        const meta = controller ? controller.device.getDualShockMeta() : null;
                        if (meta !== null) {
    ...
    private handleReport(data: { report: DualshockReport, meta: DualshockMeta }) {
        try {
            if (this.socket !== null) {
                    ...
                    for (const client of clients) {
                        this.socket.send(outBuffer, client.port, client.address, (error, bytes) => {
                            if (error) {
                                ...
                                // tslint:disable-next-line:max-line-length
                                this.emitError(new Error(`failed to completely send all of buffer. Sent: ${bytes}. Buffer length: ${outBuffer.length}`));
                            }
                        });
                    }
                }
    ...
    private getClientsForReport(meta: DualshockMeta) {
        ...
        for (const [info, times] of this.clients) {
            if (currentTime - times.timeForAllPads < ClientTimeoutLimit) {
                ...
                (currentTime - times.timeForPadById[meta.padId] < ClientTimeoutLimit)
            ) {
                clients.push(info);
            }
            else if (
                times.timeForPadByMac.has(meta.macAddress) &&
                (currentTime - times.timeForPadByMac.get(meta.macAddress)! < ClientTimeoutLimit)
            ) {
                clients.push(info);
            }
            else {
